...
function AppContent() {
  ...
  const dayStatuses = useMemo(() => {
    ...
    groupedClosures.forEach(group => {
      if (group.items.every(i => i.status === 'bank')) statuses[group.date] = 'bank';
      else if (group.items.some(i => i.status === 'transit')) statuses[group.date] = 'transit';
      else statuses[group.date] = 'safe';
    });
    return statuses;
  }, [groupedClosures]);
  ...
  const toggleDayStatus = async (dayString: string) => {
    ...
    const currentStatus = dayStatuses[dayString];
                   ...
                   {groupedClosures.map(group => (
                     <React.Fragment key={group.date}>
                        <tr onClick={() => toggleDay(group.date)} className="bg-white/[0.03] cursor-pointer hover:bg-white/[0.06] transition-colors border-y border-white/5">
                           ...
                           <td className="px-6 py-4 text-center">
                              <button onClick={(e) => { e.stopPropagation(); toggleDayStatus(group.date); }} className="px-4 py-2 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-[10px] font-black uppercase flex items-center gap-2 mx-auto">
